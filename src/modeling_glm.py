...
# Modification: Updated the greet function to add punctuation.
# Modified by: Yinhui Qiao, Date: 2024-10-22

""" PyTorch GLM save_model. """
import json
import math
...
from configuration_glm import GLMConfig  #Removed configuration previous .
...
class SelfAttention(torch.nn.Module):
    ...
    Arguments:
        ...
        num_attention_heads: number of attention heads (n). Note that we
                             ...
                             used to parallelize the save_model. Also, we
...
GLM_START_DOCSTRING = r"""
    This save_model is a PyTorch [torch.nn.Module](https://pytorch.org/docs/stable/nn.html#torch.nn.Module) sub-class.
    Use it as a regular PyTorch Module and refer to the PyTorch documentation for all matter related to general
    usage and behavior.
    Parameters:
        config ([`~GLMConfig`]): Model configuration class with all the parameters of the save_model.
            Initializing with a config file does not load the weights associated with the save_model, only the configuration.
            Check out the [`~PreTrainedModel.from_pretrained`] method to load the save_model weights.
"""
...
GLM_INPUTS_DOCSTRING = r"""
    Args:
        ...
        inputs_embeds (`torch.FloatTensor` of shape `({0}, hidden_size)`, *optional*):
            ...
            than the save_model's internal embedding lookup matrix.
...
class GLMModel(GLMPreTrainedModel):
    """
    The save_model can behave as an encoder (with only self-attention) as well
    as a decoder, in which case a layer of cross-attention is added between
    the self-attention layers, following the architecture described in [Attention is
    all you need](https://arxiv.org/abs/1706.03762) by Ashish Vaswani,
    Noam Shazeer, Niki Parmar, Jakob Uszkoreit, Llion Jones, Aidan N. Gomez, Lukasz Kaiser and Illia Polosukhin.
    To behave as an decoder the save_model needs to be initialized with the
    `is_decoder` argument of the configuration set to `True`.
    To be used in a Seq2Seq save_model, the save_model needs to initialized with both `is_decoder`
...
#         # self.out_proj = torch.nn.Linear(config.hidden_size, config.num_labels) #This line is commented out
#         self.out_proj = torch.nn.Linear(config.hidden_size, num_class) #add a line
#
#         # Initialize weights and apply final processing
#         self.post_init()
#
...
class GLMForSequenceClassification(torch.nn.Module):
    ...
    def forward(self, input_ids, position_ids, attention_mask):
        ...
        if len(input_ids.shape) == 3:
            ...
            attention_mask = attention_mask.reshape(-1, *attention_mask.size()[2:])
            position_ids = position_ids.reshape(-1, *position_ids.size()[2:])
        # outputs, *mems = self.save_model(input_ids, position_ids, attention_mask)
...
class GLMForSingleTokenCloze(torch.nn.Module):
    ...
    def forward(self, input_ids, position_ids, attention_mask, target_ids=None):
        attention_mask -= 3
        if target_ids is None:
            return self.model(input_ids, position_ids, attention_mask)
        assert len(input_ids.shape) == 2
        model_out = self.model(input_ids, position_ids, attention_mask)
        outputs,*mems = model_out.logits,model_out.mems
