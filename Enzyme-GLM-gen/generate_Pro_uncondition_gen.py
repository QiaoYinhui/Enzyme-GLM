...
def recover_smi(smi):
    encoder_dit =  {'C': '!', 'B': '<', 'F': '>', 'I': '^', 'H': '&', 'O': '*', 'N': '~', 'P': '?', 'S': '$',
                            'Cl': ',', 'Br': '.', 'Si': '{', 'Mg': '}', 'Ca': ':', 'Fe': ';', 'As': '|', 'Al': '`',
                            '*': '"'}
    decoder = {v: k for k, v in encoder_dit.items()}
    re_dit = decoder
    smiles_regex_pattern = r'{|}|:|;|||`|,|.|["#%\)\(\+\-1032547698:=@!<>^&*~?$\[\]icosn]|/|\\'
    ...
    for car in smi_list:
        if car in re_dit:
            car = re_dit[car]
            recover_smi_list.append(car)
            recover_smi_str += car
        else:
            recover_smi_list.append(car)
            recover_smi_str += car
    return recover_smi_list,recover_smi_str


def recreate_smi(smi):
    re_dit = { 'C': '!','B': '<', 'F': '>', 'I': '^', 'H': '&', 'O': '*', 'N': '~', 'P': '?', 'S': '$','Cl':',','Br':'.','Si':'{','Mg':'}','Ca':':','Fe':';','As':'|','Al':'`','*': '"'}
...
def read_context(tokenizer, args, output):
    ...
    if mpu.get_model_parallel_rank() == 0:
        while True:
            # raw_text = input("\nPlease enter the amount of enzyme you want to generate unconditionally (stop to exit) >>> ")
            # if not raw_text:
            ...
            #     break
            # ###
            # _,raw_text = recreate_smi(raw_text)
            raw_text = '[CLS]' + '[pro]'
            ###
...
def generate_samples(model, tokenizer, args, device):
    ...
    output_path = os.path.join(output_path, f"uncondition-{datetime.now().strftime('%m-%d-%H-%M')}.txt")
