...
class PromptDataset(data.Dataset):
    def __init__(self, prompt_loader, text_loader, tokenizer=None, to_tokenize=False,shengchengfenzi_fin=False, **kwargs):
        ...
        if isinstance(self.prompts, LazyLoader) and isinstance(self.texts, LazyLoader):
            ...
            self.text_lens = self.texts.lens
            self.is_lazy = True
        self.shengchengfenzi_fin = shengchengfenzi_fin
    ...
    def __getitem__(self, index):
        ...
        text = self.texts[index]
        if self.shengchengfenzi_fin:
            #print(text)
            traget = text.index(4105)
            return {"tokens": prompt + text, "loss_masks": [0] * len(prompt) +[0] + [1] * len(text[1:traget]) + [0] + [1]*len(text[traget+1:])}
            #return {"tokens": prompt + text, "loss_masks": [0] * len(prompt) + [0] + [1] * len(text[0]) + [0] + [1] * len(text[1])}
        else:
            if self.to_tokenize:
...
class KeyReader(DataReader):
    ...
    PATH = '/mnt/d/111QYH/progect/GLM/qyh_vm/GLM_Linux/data/train_text_11-15/new_shuangyv_data_20000.txt'
...
class BertLargeData(BertData):
    ...
    PATH = '/mnt/d/111QYH/progect/GLM/qyh_vm/GLM_Linux/data/train_text_11-15'

class WuDaoCorpus(PromptReader):
    #PATH = "/dataset/fd5061f6/chinese_data/WuDao"
    #PATH = "/mnt/d/111QYH/progect/GLM/qyh_vm/GLM_Linux/data/yibaiw_data"
    #PATH = "/home/qyh_vm/GLM_Linux/data/pretrain_small"
    PATH = "/home/qyh_vm/GLM_Linux/data/train_1024"
    #is_json = False
    is_json = True
    reserve_punct = True
    #split_row = False
...
class ESPair(PromptReader):
    PATH = "/mnt/d/111QYH/progect/GLM/qyh_vm/GLM_Linux/data/ENZYME_ESPair_plus"
    #is_json = False
    #reserve_punct = True
    #split_row = False
    is_json = True
    ...
    def process_line(self, item, tokenizer, tokenize):
        prompts, texts = [], []
        text = ""
        title = item.get("title", None)  # 说明item是一个字典
        content = item.get("content", None)
        if title:
            text += title.strip() + " "
        if content:
