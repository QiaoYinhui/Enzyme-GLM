    ...
    def construct_blocks(self, samples):
        ...
        if rand < self.bert_prob:
            ...
            for sample in samples:
                ...
                #print('generate_blank_data之后，数据变了')
                if data is not None:
                    if self.encoder_decoder:
        ...
        if self.encoder_decoder:
            ...
            while len(token_batch) < 4:
                # 填充为第一条数据
                token_batch.append(token_batch[0])
                target_batch.append(target_batch[0])
                loss_mask_batch.append(loss_mask_batch[0])
                position_id_batch.append(position_id_batch[0])
