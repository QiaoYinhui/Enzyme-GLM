...
def _train(model, optimizer, lr_scheduler, forward_step,
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for iteration_, batch in enumerate(train_dataloader[0]):
            ...
            lm_loss, skipped_iter, _ = train_step(data, model, optimizer, lr_scheduler, args,
                                                   timers, forward_step_func=forward_step, single_step=False)
...
def finetune(args, train_valid_datasets_provider, model_kwargs, forward_step=finetune_forward_step,
    ...
    #print(tokenizer._command_token_vocab)
    ...
    if train_valid_datasets_provider is not None and args.epochs > 0:
        if mpu.get_model_parallel_rank() == 0:
            ...
            for index,data in enumerate(train_dataloader):
                if index <= 4:
                    data_dit['text'] = data['text'][0].tolist()
                    data_dit['label'] = data['label'][0].tolist()
                    data_dit['position'] = data['position'][0].tolist()
                    data_dit['mask'] = data['mask'][0].tolist()
                    data_dit['target'] = data['target'][0].tolist()
                    data_dit['logit_mask'] = data['logit_mask'][0].tolist()
                    print('数据在这里!',data_dit)
            '''
            
            if args.no_validation:
                valid_dataloader = None
            train_iters = torch.cuda.LongTensor([len(train_dataloader)])
            #print('train_iters!!!!!!!!!!!!!!!',train_iters,type(train_iters),train_iters[1])
        else:
        ...
        if args.block_lm_ratio > 0.0:  #False
            if mpu.get_model_parallel_rank() == 0:
